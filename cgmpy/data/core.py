...
class ModularGlucoseData:
    ...
    def _initialize_data(
        ...
        t6 = time.time()
        processed_data, time_diffs = self.processor.process_data(
            data, "time", "glucose", self.log
        )
        t7 = time.time()
    ...
    def get_logs(self) -> Dict[str, Any]:
        ...
        if not self.log:
            self.logger.warning(
                "Logging no está activado. Inicialice la clase con log=True para generar logs."
            )
            return {}
        return self.logs
    ...
    def append_to_parquet(self, file_path: str, compression: str = "snappy", handle_duplicates: str = "keep_new") -> int:
    ...
    def filter_by_date_range(
        ...
        filtered_data = self.processor.filter_by_dates(
