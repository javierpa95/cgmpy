...
class DataProcessor:
    ...
    def _is_optimized_parquet(self, data: pd.DataFrame, date_col: str, glucose_col: str) -> bool:
        ...
        """
        return (
            pd.api.types.is_datetime64_any_dtype(data[date_col]) and
            data[glucose_col].dtype == "int16"
        )
    ...
    def _convert_data_types(self, data: pd.DataFrame, date_col: str, glucose_col: str) -> pd.DataFrame:
        ...
        if data_converted[glucose_col].dtype != "int16":
            ...
            min_val, max_val = data_converted[glucose_col].min(), data_converted[glucose_col].max()
            if (
                pd.notna(min_val) and pd.notna(max_val) and
                min_val >= -32768 and max_val <= 32767
            ):
    ...
    def _handle_duplicates(self, data: pd.DataFrame, date_col: str, glucose_col: str) -> pd.DataFrame:
        ...
        if num_duplicados > 0:
            ...
            return pd.concat([
                data[~mask_duplicados],
                df_dups.loc[idx_to_keep].drop(columns="diff")
            ])
