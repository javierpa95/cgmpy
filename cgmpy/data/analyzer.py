...
class DataAnalyzer:
    ...
    def get_basic_info(
        ...
        if include_disconnections:
            resumen["lista_desconexiones"] = self._get_disconnection_details(
                data, desconexiones
            )
        
        return resumen
    
    def _get_disconnection_details(
        self,
        data: pd.DataFrame,
        desconexiones: pd.Series
    ) -> list:
        """
            ...
            for idx, indice in enumerate(desconexiones.index, 1):
                try:
                    ...
                    if posicion_actual > 0:
                        ...
                        duracion_minutos = (
                            fecha_fin_desconexion - fecha_inicio_desconexion
                        ).total_seconds() / 60
