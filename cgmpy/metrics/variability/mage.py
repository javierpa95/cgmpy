...
class MAGEMetrics(VariabilityBase):
    ...
    def MAGE(self, threshold: float | None = None) -> float:
        ...
        :param threshold: Minimum excursion magnitude to count.
            Uses 1 SD of the dataset if not provided.
        :return: MAGE value.
        """
        if threshold is None:
            threshold = self.sd()
        return mage_simple(self.data["glucose"], threshold)
    ...
    def MAGE_Baghurst(self, threshold_sd: int = 1, approach: int = 1, plot: bool = False) -> dict:
        ...
        glucose_vals = self.data["glucose"]
        ...
        # Non-plot path: delegate to pure functions
        if not plot:
            fn = {
        ...
        from cgmpy.plotting.mage_excursions import plot_mage_excursions
