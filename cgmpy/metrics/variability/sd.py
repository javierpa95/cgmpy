...
class SDMetrics(VariabilityBase):
    ...
    :class:`~cgmpy.data.core.GlucoseData` interface (``data``,
    ...
    def sd_total(self) -> dict:
        ...
        """
        return {"sd": sd_global(self.data["glucose"]), "mean": mean_global(self.data["glucose"])}
    ...
    def sd_within_day(self, min_count_threshold: float = 0.5) -> dict:
        ...
        """
        return sd_within_day(self.data["glucose"], self.data["time"], min_count_threshold)
    ...
    def sdw(self, min_count_threshold: float = 0.5) -> float:
        ...
        """
        return sdw(self.data["glucose"], self.data["time"], min_count_threshold)
    ...
    def sd_between_timepoints(
        ...
        :param group_by_intervals: If True, groups data into regular time intervals.
        :param interval_minutes: Interval size in minutes for grouping (default 5 min).
    ...
    def sd_within_series(self, hours: int = 1) -> dict:
        ...
        .. note::
            ...
            **Approximation.** For performance on large datasets this method
            ...
            The result is an **estimate** of the true value, not an exact
            computation. If you need exact results on a small dataset, use a
            different metric (e.g. :meth:`sd_total`) or reduce the dataset size
            before calling this method.

        :param hours: Window size in hours
        :return: Dictionary with average SD and mean of time series
        """
        return sd_within_series(self.data["glucose"], self.data["time"], hours)
    ...
    def sd_daily_mean(self, min_count_threshold: float = 0.5) -> dict:
        ...
        """
        return sd_daily_mean(self.data["glucose"], self.data["time"], min_count_threshold)
    ...
    def sd_same_timepoint(
        ...
        :param min_count_threshold: Threshold to consider a timestamp valid
                               (proportion of count median).
        :param filter_outliers: If True, filters timestamps with few data points.
        :param group_by_intervals: If True, groups data into regular intervals.
        :param interval_minutes: Interval size in minutes (default 5 min).
    ...
    def sd_same_timepoint_adjusted(self) -> dict:
        ...
        """
        return sd_same_timepoint_adjusted(self.data["glucose"], self.data["time"])
    ...
    def sd_interaction(self) -> dict:
        ...
        """
        return sd_interaction(self.data["glucose"], self.data["time"])
    ...
    def sd_segment(self, start_time: str, duration_hours: int) -> dict:
        ...
        """
        return sd_segment(self.data["glucose"], self.data["time"], start_time, duration_hours)
    ...
    def calculate_all_cv_metrics(self) -> dict:
        ...
        Each CV is computed as ``SD(subset) / mean(subset) * 100`` where
        both SD and mean come from the **same** data subset. Verified correct:
        the numerator and denominator are always computed from identical
        groupings, so each CV formula matches its definition.
        """
        return {
    ...
    def _get_segment_data(self, start_time: str, duration_hours: int) -> pd.DataFrame:
        ...
        """
        mask = _get_segment_mask(self.data["time"], start_time, duration_hours)
