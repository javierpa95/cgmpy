...
- **`GlucoseAnalysis.targets` setter** — added a property setter that delegates to `GlucoseData.targets` and clears the cache, fixing `AttributeError` in example when switching to pregnancy targets.
