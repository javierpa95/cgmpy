...
from cgmpy.plotting import daily_plots as daily_module
...
@pytest.fixture
def gp_2day(glucose_2day_df):
    return GlucoseAnalysis(GlucoseData(data_source=glucose_2day_df))
...
# ---------------------------------------------------------------------------
# Public API — day_graph (via GlucoseAnalysis)
# ---------------------------------------------------------------------------


def test_day_graph_default_date_creates_figure(gp_24h, monkeypatch):
    """``plot_daily`` without args uses the first day in the data."""
...
def test_day_graph_explicit_date_creates_figure(gp_24h, monkeypatch):
    """``plot_daily(date="YYYY-MM-DD")`` must respect the requested day."""
    monkeypatch.setattr(plt, "show", lambda: None)
    gp_24h.plot_daily(date="2024-01-01")
    assert len(plt.get_fignums()) == 1
    ax = plt.gcf().axes[0]
...
def test_day_graph_no_data_for_date_returns_none(gp_24h, monkeypatch):
    """``plot_daily`` returns None silently if the date has no data."""
    monkeypatch.setattr(plt, "show", lambda: None)
    result = gp_24h.plot_daily(date="2099-12-31")
    assert result is None
    assert plt.get_fignums() == []


def test_day_graph_axes_have_zones_and_line(gp_24h, monkeypatch):
    """The plotted axes must contain 5 zone patches, lines, and text labels."""
    monkeypatch.setattr(plt, "show", lambda: None)
    gp_24h.plot_daily()
    ax = plt.gcf().axes[0]
...
def test_add_glucose_zones_helper():
    ...
    assert "Severe hypoglycemia" in labels
    assert "Severe hyperglycemia" in labels


def test_add_reference_lines_helper():
    fig, ax = plt.subplots()
    daily_module._add_reference_lines(ax)
...
def test_configure_daily_plot_helper():
    fig, ax = plt.subplots()
    ax.plot([0, 24], [100, 100], label="dummy")
    daily_module._configure_daily_plot(ax, "Test Title")
...
def test_configure_overlapping_plot_helper():
    """``_configure_overlapping_plot`` sets labels, zones and reference lines."""
    plt.figure()
    ax = plt.gca()
    ax.plot([0, 24], [100, 100], label="dummy")
    daily_module._configure_overlapping_plot()
    assert ax.get_title() == "Overlapping Glucose Profiles"
    assert ax.get_xlabel() == "Time of Day"
    assert ax.get_ylabel() == "Glucose Level (mg/dL)"
    assert len(ax.lines) == 3
...
def test_day_graph_with_single_day_data(monkeypatch):
    """``plot_daily`` on a 1-row DataFrame must not raise."""
    monkeypatch.setattr(plt, "show", lambda: None)
    df = pd.DataFrame(
        {
